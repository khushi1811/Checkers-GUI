...
class Checkers_Game:
    ...
    def declare_winner(self):
        ...
        elif (self.board.black <= 0 or (self.turn==black and len(self.all_moves)==0)):
...
# minimax algorithm
def final_minimax(current_board, depth, turn, game_work):
    # if depth is 3 or there is no winner the return updated board and updated score
    if (depth == 3 or game_work.declare_winner() != None):
        return current_board.utility_mark(), current_board

    # MAX player turn: If turn =1 then max = -INF and finds out possible moves and saves it in a list. 
    # Recursively calls minimax by increasing the depth and
    #  switching the turn and runs iteration of the list if max<val then save and return best possible move
    if (turn == 1):
        moved_boards = get_moved_boards(current_board, beige, game_work)
        MAX = -INF
        best_moved_board = None
        for i in range(len(moved_boards)):
            val, temp = final_minimax(moved_boards[i], depth + 1, 0, game_work)
            if (MAX < val):
                best_moved_board = moved_boards[i]
                MAX = val
        return MAX, best_moved_board

    # MIN player turn: Else min = INF and finds out possible moves and saves it in a list.
    #  Recursively calls minimax by increasing the depth without switching the turn and 
    # runs an iteration of the list if min>val then saves and returns the best possible move.
    else:
        moved_boards = get_moved_boards(current_board, black, game_work)
        MIN = INF
        best_moved_board = None
        for i in range(len(moved_boards)):
            val, temp = final_minimax(moved_boards[i], depth + 1, 1, game_work)
            if (MIN > val):
                best_moved_board = moved_boards[i]
                MIN = val
...
def main():
    ...
    while run:
        ...
        if (game_work.turn == beige):
            value, new_board = final_minimax(game_work.get_board(), 0, 1, game_work)
